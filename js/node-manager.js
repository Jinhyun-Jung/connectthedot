...
function showFolderLinkDialog(node, x, y) {
    ...
    const escapeFolderDialogHtml = (value) => String(value ?? '').replace(/[&<>"']/g, char => ({
        '&': '&amp;',
        '<': '&lt;',
        '>': '&gt;',
        '"': '&quot;',
    ...
    // 현재 노드의 폴더 경로
    const currentFolder = normalizeFolderPath(node.folder || '/');
    const sourceFolderPath = node.isFolder
        ? normalizeFolderPath(node.folderPath || `/${node.title || ''}`)
        : null;

    const availableFolderEntries = folderNodes
        .filter(folder => folder.id !== node.id)
        .map(folder => {
            const path = normalizeFolderPath(folder.folderPath || `/${folder.title || ''}`);
            return {
                folder,
                path,
        ...
        // 폴더를 자기 하위 폴더에 연결하면 순환 구조가 되므로 목록에서 제외한다.
        .filter(entry => !sourceFolderPath || !entry.path.startsWith(`${sourceFolderPath}/`));

    // 기존 최상위 폴더 순서는 유지하면서 각 하위 폴더를 부모 바로 뒤에 배치한다.
    const availablePaths = new Set(availableFolderEntries.map(entry => entry.path));
    const childrenByParentPath = new Map();
    availableFolderEntries.forEach(entry => {
        const actualParentPath = entry.segments.length > 1
            ? `/${entry.segments.slice(0, -1).join('/')}`
            : '/';
        const visibleParentPath = availablePaths.has(actualParentPath) ? actualParentPath : '/';

        if (!childrenByParentPath.has(visibleParentPath)) {
            childrenByParentPath.set(visibleParentPath, []);
        }
        childrenByParentPath.get(visibleParentPath).push(entry);
    });

    const folderEntries = [];
    const appendedFolderIds = new Set();
    const appendFolderChildren = (parentPath) => {
        (childrenByParentPath.get(parentPath) || []).forEach(entry => {
            if (appendedFolderIds.has(entry.folder.id)) return;
            appendedFolderIds.add(entry.folder.id);
            folderEntries.push(entry);
            appendFolderChildren(entry.path);
        });
    };
    appendFolderChildren('/');

    // 잘못된 기존 경로가 있더라도 연결 대상에서 빠지지 않게 마지막에 보완한다.
    availableFolderEntries.forEach(entry => {
        if (!appendedFolderIds.has(entry.folder.id)) folderEntries.push(entry);
    });
    ...
    // 폴더 노드들
    folderEntries.forEach(({ folder, path, segments }) => {
        const folderColor = folder.borderColor || getFolderColor(path);
        const isSelected = currentFolder === path;
        const depth = Math.max(0, segments.length - 1);
        const title = folder.title || segments[segments.length - 1] || '이름 없음';

        folderListHtml += `
            <div class="folder-link-item ${isSelected ? 'selected' : ''}" data-path="${escapeFolderDialogHtml(path)}" data-folder-id="${escapeFolderDialogHtml(folder.id)}" data-depth="${depth}" style="--folder-link-indent: ${depth * 20}px;">
                <span class="folder-dot" style="background: ${folderColor}; width: 10px; height: 10px; border-radius: 50%; display: inline-block;"></span>
                <span>${escapeFolderDialogHtml(title)}</span>
            </div>
        `;
    });
    ...
    style.textContent = `
        .folder-link-item {
            ...
            gap: 8px;
            padding: 8px 10px;
            padding-left: calc(10px + var(--folder-link-indent, 0px));
            border-radius: 4px;
            cursor: pointer;
    ...
    requestAnimationFrame(() => {
        ...
        nodesToRender.forEach(node => {
            ...
            if (existingElement) {
                ...
                existingElement.classList.toggle('highlighted-node', !!node.highlighted);
